    ...
    if (! in_array($row->id, $agregadas)) {
        ...
        if ($resultado->num_rows) {
            ...
            while ($actor = $resultado->fetch_object()) {
                printf('<div class="pull-left margin-right-10"><a href="index.php?page=actores&id=%u" title="%s"><img class="rounded-circle mx-auto d-block" src="photos/actores/40/%u.jpg" /></a></div>', $actor->id, $actor->nombre, $actor->id);
            }
            echo '</div>';
        }
        $resultado->close();
